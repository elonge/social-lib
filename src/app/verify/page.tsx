...
import { Check, Edit2, Trash2, Plus, Sparkles, Loader2, RefreshCw, AlertCircle } from "lucide-react";
...
export default function VerifyPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      // Filter out duplicates if desired, or just save all (and let backend handle, but we'll save all for now)
      // The requirement says "Excludes already-owned books from the staged list (or marks them as duplicates)"
      // We will allow saving duplicates but warn the user.
      const result = await saveBooksToLibrary(books);
      if (result.success) {
        // Clear capture data from IndexedDB
  ...
  const handleConfirmClick = () => {
     // Mock auth check: always show modal for this phase
  ...
  const handleRefetchCover = async (book: Book) => {
    ...
    try {
      ...
      console.log("Refetched cover for", book.title, ":", cover);
      if (cover.url) {
              ...
              <div key={book.id} className={cn(
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    {book.isUnidentified && (
                      <span className="px-2 py-0.5 bg-amber-200 text-amber-800 text-[10px] uppercase font-bold tracking-wider rounded-full">
                        Unidentified
                      </span>
                    )}
                    {duplicate && (
